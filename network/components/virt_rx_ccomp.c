...
void virt_rx_ccomp_rx_return(void)
{
    ...
    while (reprocess) {
        while (!net_queue_empty_active(&state.rx_queue_drv)) {
            ...
            _ccomp_cache_clean_and_invalidate(buffer_vaddr, buffer_vaddr + buffer.len);
            int client = virt_rx_ccomp_get_mac_add_match((struct ethernet_header *) buffer_vaddr);
            if (client == BROADCAST_ID) {
                int ref_index = buffer.io_or_offset / NET_BUFFER_SIZE;
                _ccomp_assert(buffer_refs[ref_index] == 0);
    ...
    for (int client = 0; client < config.num_clients; client++) {
        ...
        while (reprocess) {
            while (!net_queue_empty_free(&state.rx_queue_clients[client])) {
                ...
                _ccomp_assert(!err);
                _ccomp_assert(!(buffer.io_or_offset % NET_BUFFER_SIZE)
...
{
    virt_rx_ccomp_rx_return();
    virt_rx_ccomp_rx_provide();
}
