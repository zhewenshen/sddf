...
// - ARP performs set check protocol but only for RX used ring (not TX free as the assumption is that generally there will always be enough
// TX free buffers). Note that CLT_TX_free notify flag is never set.
// OUTCOME: Verifies in 10.1 seconds
...
active proctype ARP() priority 1 {
    do
        :: CLT_RX_used.notification ? 1 ->
            ...
            if
                :: transmitted && CLT_TX_used.notify_flag && !(CLT_TX_used.notification ?? [1]) -> 
            ...
            if
                :: received && CLT_RX_free.notify_flag && !(CLT_RX_free.notification ?? [1]) ->
